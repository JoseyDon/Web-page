...
1. clickclose函数在页面加载时调用，cookieget函数判断cookie是否已经点击了不再提醒Y：隐藏 N：显示
2. （在显示基础上）点击关闭按钮触发closeclick函数，div隐藏同时调用cookieget函数。
### 1.2关注“网易教育产品部”/登录
1. 点击关注按钮：首先判断登录的cookie是否已设置（loginSuc）
2. 如果未设置登录cookie，则弹出登录框，使用给定的用户名和密码（需要表单验证）调用服务器Ajax登录，成功后设置登录cookie
3. 登录成功后，调用关注API，并设置关注成功的cookie（followSuc）
4. 登录后“关注”按钮变成不可点的“已关注”状态。按钮的hover效果见视觉稿
